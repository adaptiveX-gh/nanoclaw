...
- **Issue:** Freqtrade MCP (Python) and StrategyDNA MCP (Python) always initialized, even for non-trading queries. Each Python process imports heavy dependencies (freqtrade, ta-lib).
- **Impact:** 5-10 seconds startup penalty per container
- **Fix:** Make MCP servers conditional on env flags (like TDS already is). Or lazy-spawn Python MCPs on first tool call.
